...
- Объекты Kubernetes, `RollingUpdate`, пробы — [`KUBERNETES.md` §5, §10](../../infrastructure/theory/KUBERNETES.md). Чем и кто применяет изменение — [`DEPLOY_K8S_AND_SWARM.md` §1](DEPLOY_K8S_AND_SWARM.md) и [`DELIVERY_PUSH_VS_PULL.md` §1](DELIVERY_PUSH_VS_PULL.md).
...
Штатная стратегия Deployment заменяет реплики пачками и не умеет остановиться на 10% и подождать. Ручек ровно две — `maxSurge` и `maxUnavailable`, обе про скорость замены, ни одной про долю трафика.
...
Два Deployment с общей меткой, на которую смотрит Service, и разным числом реплик. Трафик делится не потому, что его кто-то делит, а потому, что kube-proxy раскидывает соединения по endpoint'ам примерно поровну.
...
**Следствие.** Способ работает там, где реплик много, а доли грубые. На трёх подах «канарейка 5%» — самообман: минимум, который вы выдадите, — 33%.
...
Тот же `HTTPRoute` выбирает бэкенд не по случайности, а по признаку запроса: заголовку, куке, значению из токена.
...
Различие не академическое: канарейку катят автоматически, эксперимент — нет. Пятиминутного окна не хватит ни одной продуктовой метрике, а «плохая конверсия» не повод для автоматического отката.
...
`AnalysisTemplate` превращает «посмотрели графики» в условие, которое проверяет контроллер: шаг `analysis` блокирует раскатку до вердикта, а анализ в `strategy.canary.analysis` идёт фоном.
...
Ошибка здесь не в синтаксисе, а в арифметике. При 40 запросах в секунду и весе 5% на канарейку идёт два запроса в секунду, за минуту — 120. Рост доли ошибок с 0.1% до 1% означает 0.12 ожидаемой ошибки против 1.2 — это шум. На весе 25% за десять минут те же пороги дают 6000 запросов и 60 ошибок против 6: сигнал.
...
- **Липкие сессии.** Если пользователь закреплён за подом, доля трафика перестаёт быть долей пользователей, а откат выбрасывает сессии.
- **Асинхронная работа.** Потребитель очереди получает сообщения, а не долю трафика: вес в маршруте на него не действует.
...
**Дерево решений.** Нет метрики здоровья → нужна метрика, а не канарейка. Метрика есть, трафика мало → blue-green с ручным переключением. Есть и метрика, и трафик → канарейка с анализом. Нужен сегмент, а не доля → это эксперимент, а не релиз.
