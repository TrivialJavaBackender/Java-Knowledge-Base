...
В каждом регионе — свой Redis-кластер. Состояние между регионами не реплицируется (добавило бы задержка и сложности).
...
Нюанс: пользователь, бьющий по нескольким регионам, не оценивается глобально. Приемлемый компромисс ради низкой задержка.

Для глобальных лимитов (редко) — центральный Redis-кластер или агрегирующий сервис (за счёт дополнительной задержка).
...
1. **Fail open** (разрешить всё) — лучше UX, но уязвимость к burst-атакам;
