...
Клиент генерирует уникальный ключ (UUID) и передаёт с каждым мутирующим запросом. Сервер запоминает `(key → result)` и при повторе возвращает сохранённый ответ без повторного выполнения. Защищает от дублей при retry после network timeout.

Детальный разбор (хранение БД vs Redis, коллизии, TTL, in-flight дедупликация) — в [`RELIABILITY_PATTERNS.md`](RELIABILITY_PATTERNS.md#ключи-идемпотентности).
...
N=3, W=1, R=1 → R+W=2 ≤ 3 → eventual, максимальная пропускная способность
