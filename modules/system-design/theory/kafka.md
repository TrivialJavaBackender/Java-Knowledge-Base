...
```
Защищает от дублей из-за retry продюсера, но не защищает от дублей на стороне consumer.
...
Производитель и потребитель деплоятся независимо. Схема сообщения (события) меняется со временем — добавляются поля, переименовываются, разбиваются. В логе одновременно живут события **разных версий**, а потребители тоже могут быть разных версий.
...
|---|---|---|---|
| Модель | Pull (потребитель тянет) | Push (брокер толкает) | Pull |
| Retention | Дни/недели, переигрывание | Удаляет после ACK | До 14 дней |
| Порядок | Внутри партиции | Очередь FIFO (1 потребитель) | Без гарантий |
| Пропускная способность | Очень высокий | Средний | Высокий (managed) |
| Переигрывание | Да (перемотать offset) | Нет | Нет |
| Паттерн | Event streaming, log | Task queue, routing | Слабая связность сервисов |
...
```java
// acks=all: лидер ждёт подтверждения от всех ISR перед ответом продюсеру
props.put("acks", "all");
...
До Kafka 3.3: метаданные хранились в **ZooKeeper** — отдельный сервис распределённой координации.
