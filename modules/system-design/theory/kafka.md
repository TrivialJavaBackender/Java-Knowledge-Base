...
Producer и Consumer релизятся независимо. Схема сообщения (события) меняется со временем — добавляются поля, переименовываются, разбиваются. В логе одновременно живут события **разных версий**, а потребители тоже могут быть разных версий.
