...
> **Какую проблему решает.** Даёт словарь и рабочие инструменты: что такое корутина, что делает
> `suspend`, какой билдер когда брать.
> **Кому это надо.** Всем; это базовый файл модуля — без него остальные не имеют смысла.
> **Когда НЕ надо.** Незачем читать вперёд про внутренности: [`SUSPEND_INTERNALS.md`](SUSPEND_INTERNALS.md)
> пригодится, только когда появятся вопросы «а как это вообще работает».
>
> Если непонятно, зачем корутины нужны и чем они лучше колбеков и `CompletableFuture`, начните с
...
### `join()` против `await()` при ошибке

Разница, на которой легко ошибиться:

```kotlin
val job = scope.launch { error("boom") }
job.join()          // НЕ бросает: join ждёт завершения, каким бы оно ни было

val deferred = scope.async { error("boom") }
deferred.await()    // бросает IllegalStateException("boom")
```

`join()` возвращает `Unit` и не сообщает о причине завершения — узнать её можно через
`job.isCancelled` или `job.invokeOnCompletion { cause -> … }`. При этом само исключение из `launch`
уже улетело вверх по дереву и, скорее всего, отменило родителя — то, что `join()` промолчал, не
значит, что ошибка потерялась (см. [`CANCELLATION_EXCEPTIONS.md`](CANCELLATION_EXCEPTIONS.md)).

### Где эти состояния видно на практике

Таблица состояний нужна не для зубрёжки, а в трёх местах:

1. **`invokeOnCompletion { cause -> … }`** — единственный способ узнать, чем закончилась корутина,
   не дожидаясь её через `await`: `cause == null` — успех, `CancellationException` — отмена,
   что-то иное — ошибка. Так вешают очистку ресурсов и метрики.
2. **Диагностика зависаний.** `Cancelling` (`isActive == false`, `isCompleted == false`,
   `isCancelled == true`) — это «отменили, но корутина ещё не завершилась». Долгое зависание в
   этом состоянии означает некооперативный код или очистку без `NonCancellable`.
3. **`Completing`** — родитель уже отработал своё тело, но ждёт детей. Если `job.isActive == true`
   у корутины, тело которой давно закончилось, — где-то остался незавершённый ребёнок.

### `CoroutineStart` — когда стартовать
...
|---|---|---|
| `DEFAULT` | планируется сразу, выполняется по правилам диспетчера | обычный случай |
| `LAZY` | не стартует, пока не позвали `start()`, `join()` или `await()` | подготовить работу заранее, запустить по условию |
| `ATOMIC` | стартует и **не может быть отменён до первой точки приостановки** | когда в начале блока захватывается ресурс, который потом надо освободить в `finally`: при `DEFAULT` корутину могут отменить **до** входа в блок, и `finally` не выполнится |
| `UNDISPATCHED` | выполняется немедленно в текущем потоке до первой приостановки, дальше — по диспетчеру | когда начало блока обязано произойти синхронно: подписаться на источник до того, как продолжится вызывающий код (иначе успеешь пропустить события) |

`UNDISPATCHED` — не «синхронный запуск»: после первой приостановки корутина возвращается к обычным
правилам диспетчера.

---

## 7. Правила дизайна suspend-API

Когда пишете свои suspend-функции, а не только вызываете чужие.

**1. Не навязывайте диспетчер вызывающему.** Если функция ничего не блокирует, оставьте её
контекстно-нейтральной — пусть вызывающий решает, где её исполнять:

```kotlin
// Лишнее: вызывающий уже мог быть на нужном диспетчере, теряем fast path
suspend fun parse(s: String): Dto = withContext(Dispatchers.Default) { … }
```

Обратное правило для блокирующей работы: если функция **блокирует поток**, она обязана сама увести
это на подходящий диспетчер. Вызывающий не должен знать, что внутри JDBC.

```kotlin
suspend fun findById(id: Long): User? = withContext(io) { runInterruptible { jdbc.query(…) } }
```

**2. Диспетчер — параметр, а не константа.** `class Repo(private val io: CoroutineDispatcher = Dispatchers.IO)`.
Иначе тест невозможно перевести на виртуальное время (см. [`TESTING_INTEROP.md`](TESTING_INTEROP.md)).

**3. `suspend fun` для одного результата, `Flow` для многих.** Если функция возвращает `Flow` из
одного элемента — это должна была быть `suspend fun`. Если `suspend fun` возвращает `List`, который
собирается долго и постранично, — вероятно, нужен `Flow` (см. [`FLOW.md`](FLOW.md)).

**4. Не помечайте `suspend` то, что не приостанавливается.** Это не ошибка, но ограничение для
вызывающих: обычная функция больше не сможет её позвать. `suspend` — часть контракта, а не украшение.

**5. Не принимайте `CoroutineScope` параметром функции.** Функция, которая запускает корутины в
чужом scope, скрывает от вызывающего, что работа продолжится после её возврата. Либо `suspend fun`
(вызывающий сам решает, где её запустить), либо метод класса, у которого есть собственный scope с
понятным жизненным циклом.

**6. Отмена — это ответственность функции.** Всё, что захватывает ресурс, обязано освобождать его в
`finally`; всё, что ловит `Throwable`, обязано пробрасывать `CancellationException`.

---

## 8. Типичные ошибки

### 8.1 Запуск из `GlobalScope`
...
### 8.2 Игнорирование `Job` от `launch`
...
### 8.3 Блокирующие вызовы в `Dispatchers.Default`
...
### 8.4 `async` без `await` без `coroutineScope`
