...
### Supervisor не действует на «внуков»

Изоляция распространяется **только на прямых детей** supervisor-скоупа. Дальше по дереву работают
обычные правила:

```kotlin
supervisorScope {                 // supervisor
    launch {                      // ПРЯМОЙ ребёнок — изолирован
        launch { error("boom") }  // внук: обычный Job → уронит своего родителя,
        launch { work() }         //   а значит и этого соседа-внука
    }
    launch { survives() }         // сосед первого уровня — жив
}
```

Почему так: `supervisorScope` подставляет `SupervisorJob` только в свой собственный контекст.
Внутренний `launch` создаёт обычный `Job`, и для его детей действует стандартное распространение
ошибки вверх.

Практический вывод: если нужна изоляция на каждом уровне — supervisor нужен на каждом уровне
(`supervisorScope` внутри, либо `try/catch` в теле каждого `launch`). Один supervisor на вершине
дерева не защищает всё дерево.
