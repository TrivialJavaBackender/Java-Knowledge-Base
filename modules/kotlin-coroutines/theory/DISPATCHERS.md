...
> **Какую проблему решает.** Отвечает на единственный вопрос: **где** возобновлять корутину. Не
> «где выполняется `suspend`» — само ключевое слово о потоках ничего не говорит.
> **Кому это надо.** Тому, у кого в коде есть блокирующие вызовы, пул соединений к БД или внешний
> API с лимитом — то есть практически любому бэкенду.
> **Когда НЕ надо.** Если весь код неблокирующий и живёт в одном пуле, диспетчер можно вообще не
> указывать: он унаследуется.
>
> Под капотом диспетчер — это `ContinuationInterceptor`, элемент контекста
> ([`SCOPE_CONTEXT.md`](SCOPE_CONTEXT.md)), который перехватывает каждое возобновление и решает,
> в какую очередь положить продолжение ([`SUSPEND_INTERNALS.md`](SUSPEND_INTERNALS.md) §4).
...
Полная цепочка «кто и как возобновляет корутину» — в
[`SUSPEND_INTERNALS.md`](SUSPEND_INTERNALS.md) §4. Здесь важны два следствия:

- Задачи диспетчера разбирают воркеры общего планировщика (`CoroutineScheduler`) — потоки с именами
  вида `DefaultDispatcher-worker-3`. Планировщик умеет work-stealing, поэтому «свободный» воркер
  забирает задачи у загруженного.
- `Dispatchers.Unconfined` — единственный, у которого `isDispatchNeeded == false`: продолжение
  выполняется прямо на потоке того, кто позвал `resume`. Отсюда непредсказуемый поток после каждой
  приостановки и правило «в проде почти никогда».

---

## 7. Диспетчер на виртуальных потоках (JDK 21)

Любой `Executor` превращается в диспетчер:

```kotlin
Executors.newVirtualThreadPerTaskExecutor().asCoroutineDispatcher().use { vt ->
    withContext(vt) { blockingJdbcCall() }   // блокировка виртуального потока дёшева
}
```

Зачем это может понадобиться: блокирующий вызов перестаёт быть дорогим, потому что паркуется
виртуальный поток, а не поток платформы. Лимит при этом всё равно нужен — его роль возьмёт пул
соединений или семафор, иначе в БД полетит неограниченное число запросов.

Что это **не** отменяет: структурную конкурентность, кооперативную отмену и `Flow` — их у
виртуальных потоков нет. Рабочая комбинация — корутины как модель конкурентности плюс виртуальные
потоки как исполнитель блокирующих вызовов.

> Сами виртуальные потоки (устройство, pinning, `StructuredTaskScope`) — тема JVM-уровня и живёт в
> [`concurrency/theory/VIRTUAL_THREADS.md`](../../concurrency/theory/VIRTUAL_THREADS.md).
> Здесь — только про то, как подключить их к корутинам.

Не забывайте `close()`/`use`: `asCoroutineDispatcher()` оборачивает ваш executor, и закрывать его
надо руками.

---

## 8. Тестовые диспатчеры
...
## 9. Особенности `Dispatchers.IO` после Kotlin 1.7+
