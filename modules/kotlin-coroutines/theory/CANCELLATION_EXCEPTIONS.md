...
> **Какую проблему решает.** Отмена — это способ не платить за работу, результат которой уже никому
> не нужен, и гарантированно освободить захваченные ресурсы.
> **Кому это надо.** Всем, кто пишет сервис: клиент отваливается, таймауты истекают, деплой
> останавливает под.
> **Когда НЕ надо.** Для короткого вычисления без ресурсов отмена ничего не даст — она всё равно
> сработает только в точке приостановки, которой там нет.

## Зачем отмена нужна на бэкенде

Разберём на деньгах, а не на абстракциях. Обработчик собирает страницу из трёх вызовов; клиент
закрыл соединение через 100 мс.

Без отмены три запроса доработают до конца. Это значит: три соединения из пула держатся ещё
200 мс; три вызова во внешние сервисы оплачены и посчитаны в их квоте; поток (если код блокирующий)
занят. Под нагрузкой в 1000 rps «немного лишней работы» превращается в исчерпанный пул и каскадные
таймауты у соседей.

С отменой: `coroutineScope` отменяет незавершённые вызовы, соединения возвращаются в пул, внешние
запросы прерываются. Три следствия, которые и делают эту главу обязательной:

1. **Отмена — это освобождение ресурсов**, поэтому очистка в `finally` не менее важна, чем сама
   отмена (§3).
2. **Дедлайн — это бюджет запроса.** `withTimeout` на границе обработчика превращает «сколько
   получится» в «не дольше, чем есть смысл»: клиент всё равно уже не дождётся.
3. **Проглоченная отмена ломает всю цепочку.** Один `catch (e: Exception)` без проброса — и
   `cancelAndJoin()` где-то наверху зависает, а плавное завершение работы (graceful shutdown) перестаёт работать (§2).

Механика: отмена в корутинах **кооперативная** — `cancel()` только переводит `Job` в состояние
`Cancelling`, а реально прерывает выполнение `CancellationException`, брошенное в ближайшей точке
приостановки. Исключения распространяются вверх по иерархии `Job`, но `CancellationException` —
особенный.
...
### Первое исключение — основное, остальные в `suppressed`

Если после первой ошибки падают и другие дети (например, их очистка в `finally` тоже бросила),
наружу уйдёт **первое** исключение, а последующие будут добавлены к нему через
`Throwable.addSuppressed`:

```kotlin
try {
    coroutineScope {
        launch { throw IllegalStateException("первый") }
        launch { delay(10); throw IOException("второй") }
    }
} catch (e: Throwable) {
    println(e)                 // java.lang.IllegalStateException: первый
    println(e.suppressed.size) // остальные лежат здесь
}
```

Почему это важно на практике: в логах виден только верхний `e.message`, и настоящая причина
(например, `SQLException` из закрытия транзакции) оказывается в `suppressed`. Логировать надо
исключение целиком (`log.error("...", e)`), а не `e.message` — иначе часть картины теряется.

`CancellationException`, прилетевшие из-за отмены соседей, в `suppressed` не попадают: отмена не
считается ошибкой.
