...
Защита:
- **Всегда** ставить `-XX:MaxMetaspaceSize=256m` (или сколько нужно). Лучше явный OOM с heap dump, чем медленная деградация узла.
...
Это **до того, как приложение что-то сделало**. Поэтому платформенные потоки — фундаментальный ботлнек для high-concurrency I/O: тысячи блокирующих HTTP-вызовов = тысячи потоков = десятки GB только под стэки.
