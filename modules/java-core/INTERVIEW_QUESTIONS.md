...
### Q36: Почему `intern()` в горячем цикле может быть медленнее, чем оставить дубликаты, и чем `G1 String Deduplication` принципиально отличается от `intern()`?

**A:** `intern()` делает поиск в едином процесс-широком StringTable — это фактически синхронизированный доступ к общей хеш-таблице, и вызов в горячем цикле превращает создание строк в конкурентный bottleneck на всех потоках сразу, даже притом что интернированные строки никогда не освобождаются (пул общий на JVM, а не per-classloader — переживает даже redeploy webapp). `G1 String Deduplication` работает иначе: он запускается оппортунистически **во время** young-паузы GC, сканирует уже живые (пережившие хотя бы одну сборку) String-объекты и сливает их дублирующиеся backing-массивы char/byte[], не трогая identity объектов — `==` между такими строками всё равно останется `false`. Цена нулевая на пути аллокации и не требует lock'а вообще, но выигрыш проявляется только для строк, которые пережили GC, а не сразу в момент создания.

> theory/STRING_INTERNALS.md §3
...
### Q37: Почему фреймворки типа Jackson или Spring вызывают `setAccessible(true)` один раз при старте и кэшируют `Field`/`Method`, а не делают это перед каждым использованием?

**A:** `setAccessible(true)` отключает access-check не глобально, а только на **конкретном** объекте `Field`/`Method`/`Constructor` — флаг живёт на этом объекте всё время его жизни. Сама проверка доступа, которую отключает `setAccessible`, иначе выполнялась бы на **каждом** реflective-вызове (`get`/`set`/`invoke`). Значит, вызвав `setAccessible(true)` один раз и закэшировав именно этот объект, фреймворк амортизирует стоимость отключения проверки на все будущие вызовы. Если вместо этого каждый раз заново делать `getDeclaredField` и `setAccessible`, пришлось бы повторно платить и за сам lookup, и за setAccessible — то есть кэшировать нужно оба шага сразу, а не только знание «какое поле нужно».

> theory/REFLECTION_HANDLES.md §5
...
### Q38: Чем `IllegalAccessError` из-за strong encapsulation в JPMS отличается от обычного нарушения `private`, и почему `public`-класс в non-exported пакете всё равно недоступен?

**A:** Классический контроль доступа (`public`/`private`/`protected`/package-private) — это compile-time понятие, проверяемое javac и верификатором **на уровне отдельного члена**. Strong encapsulation модуля — это более сильная граница **на уровне всего пакета**, которую обеспечивает сама модульная система независимо от видимости членов: даже полностью `public`-класс с `public`-методами в пакете, который модуль не `exports`, попросту невидим снаружи — компиляция падает так, будто пакета не существует, а если обойти это рефлексией, рантайм бросает `IllegalAccessError`/`InaccessibleObjectException` на границе модуля, ещё до того, как вообще проверяется видимость конкретного члена. Именно поэтому, в отличие от старого `private`, локальный `setAccessible(true)` здесь не помогает — модульная система требует явного `--add-opens` (или `opens` в дескрипторе), который выдаёт **владелец модуля**, а не вызывающий код, — это переворачивает то, кто контролирует лазейку.

> theory/JPMS_MODULES.md §3
...
### Q39: Почему компилятор запрещает `list.add(x)` для `List<? extends Number> list`, но разрешает `Number n = list.get(0)`?

**A:** `? extends Number` означает, что компилятор знает лишь: список хранит **какой-то** конкретный подтип `Number`, который capture conversion превращает во «внутренний» неизвестный тип `CAP#1 <: Number`. Чтение безопасно: любой элемент гарантированно приводим к `Number` независимо от того, чем на самом деле является `CAP#1` — поэтому `get()` типизирован как `Number` («producer»). Запись небезопасна: чтобы вызвать `add(x)`, компилятору нужно доказать, что `x` — это именно `CAP#1`, а он этого доказать не может (список может реально быть `List<Integer>`, а вы пытаетесь добавить `Double`) — поэтому `add` запрещён для любого wildcard, кроме `null`. Consumer-wildcard (`? super T`) переворачивает картину: безопасен только `add(T)` (нижняя граница гарантирует, что список примет хотя бы `T`), а `get()` может быть типизирован только как `Object`, потому что конкретный нижний тип неизвестен. Это и есть правило PECS — но оно прямое следствие capture conversion, а не мнемоника для запоминания.

> theory/GENERICS_ERASURE.md §5
...
### Q40: Почему `IdentityHashMap` игнорирует переопределённые `equals()`/`hashCode()` своих ключей, и что при этом кэширует HotSpot?

**A:** `IdentityHashMap` намеренно использует `System.identityHashCode()` и сравнение по ссылке (`==`) вместо вызова `equals`/`hashCode` самого объекта — это отдельный контракт, а не баг. HotSpot кэширует identity-hash в **mark word** объекта (биты в заголовке) при первом вычислении, поэтому повторные вызовы дёшевы — но это фиксирует объект: до удаления biased locking (JDK 15+/18+) это также делало объект «не-biasable». Практическое применение — отличать content-equal, но различные по identity объекты: обнаружение циклов при обходе графов (сериализация, клонирование), listener-реестры, где content equality ошибочно схлопнула бы разные экземпляры в один ключ.

> theory/EQUALS_HASHCODE_COMPARABLE.md §4
...
### Q41: Зачем понадобился JEP 358 (Helpful NPE), если `NullPointerException` и так печатает номер строки?

**A:** В одной строке может быть несколько цепочечных вызовов (`user.getProfile().getAddress().getCity()`) — номер строки и метода не говорит, **какое именно** разыменование дало `null`. JEP 358 анализирует байткод в точке throw и по verifier-информации (типы локальных переменных и стека уже известны после verification) декомпилирует ровно настолько, чтобы назвать точное выражение, вернувшее `null`, прямо в тексте сообщения. Механизм требует debug-информации (ломается при компиляции с `-g:none`) и включён по умолчанию начиная с Java 15 (`-XX:+ShowCodeDetailsInExceptionMessages`).

> theory/EXCEPTION_INTERNALS.md §5

---

### Q42: Что долетит из `try { throw new RuntimeException("A"); } finally { throw new IllegalStateException("B"); }`, и чем это принципиально отличается от try-with-resources?

**A:** Долетит `IllegalStateException("B")`, а `RuntimeException("A")` будет **бесследно потеряно**: обычный `finally` не подавляет и не накапливает исключения — второй `throw` в `finally` просто затирает то, что уже разворачивалось через стек, точно так же, как это сделал бы явный `return` в `finally`. Контраст — try-with-resources: он компилируется в структурный вызов `close()`, который явно ловит исключение при закрытии и вызывает `primary.addSuppressed(closeEx)` вместо того, чтобы дать ему затереть первичное — этот механизм появился именно потому, что обычный `finally` молча теряет данные, и это был частый источник багов до Java 7.

> theory/EXCEPTION_INTERNALS.md §6
...
### Q43: Почему в compact-конструкторе record можно переприсвоить параметр (`x = Math.abs(x)`), но нельзя написать `this.x = ...`?

**A:** Компилятор добавляет неявные присваивания полям (`this.x = x; this.y = y;`) **после** тела compact-конструктора — на момент выполнения тела final-поля ещё не инициализированы, поэтому `this.x = ...` там попросту не скомпилируется (присвоить ещё не инициализированное final-поле можно только через синтетический эпилог, который добавляет сам компилятор). Переприсвоить **параметр** `x` можно свободно — на этом этапе это обычная локальная переменная, и то значение, которое `x` содержит к концу тела compact-конструктора, компилятор скопирует в поле сгенерированным присваиванием. Отсюда правило: compact-конструктор годится для валидации и нормализации входных значений, но не для произвольной логики над полями — вы влияете на то, ЧТО присвоится, но не можете отменить или переупорядочить само присваивание.

> theory/MODERN_JAVA_FEATURES.md §2
...
### Q44: Чем `Externalizable` отличается от `Serializable` по производительности и почему он не даёт автоматической обратной совместимости?

**A:** Стандартный `Serializable` рефлексивно обходит **все** non-static/non-transient поля объекта и попутно пишет в поток метаданные о полях (описанные в *Object Serialization Specification*), что даёт хоть какую-то устойчивость к структурной эволюции класса между версиями. `Externalizable` требует вручную реализовать `writeExternal`/`readExternal` — вы пишете **только то, что выбрали**, в произвольном порядке и без дескрипторов полей в потоке. Это убирает почти весь reflection-оверхед (быстрее), но и убирает страховку: добавили или переставили поле — старые потоки будут молча прочитаны в неправильном порядке, потому что сверять байты не с чем. `Externalizable` также обязывает иметь **public no-arg конструктор** — десериализация должна суметь создать объект до того, как `readExternal` заполнит его поля, в отличие от default-механизма `Serializable`, который обходит конструктор самого класса через reflection у первого non-serializable предка.

> theory/SERIALIZATION.md §2
