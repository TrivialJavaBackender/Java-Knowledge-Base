...
## 2. Почему дефолтный `Object.equals` рвётся
...
## 3. Проблема сгенерированного БД `id`
...
## 4. Почему мутабельные поля рвут hash-коллекции
...
## 5. Бизнес-ключ как правильная основа
...
## 6. Proxy-safe equals/hashCode: почему `getClass()` ломается
...
## 7. Канонический рецепт (Vlad Mihalcea)
...
## 8. Lombok `@Data`/`@EqualsAndHashCode` на сущностях — почему опасно
