...
**A:** «Класс должен иметь одну причину для изменения», а не «класс должен делать одну вещь». Причина = ось изменчивости, конкретный stakeholder/use case. Пример: `Report` класс с методами `calculate()` и `print()` нарушает SRP, потому что меняется и при изменении формулы (accounting), и при изменении формата вывода (UI) — это разные stakeholders. Разделить на `ReportCalculator` (accounting логика) и `ReportPrinter` (UI). SRP анализируется через вопрос «кто будет требовать изменения?».
...
```
Новый тип = новый класс, метод не трогаем. Trade-off: добавляет abstraction overhead, оправдан когда есть >3 типов или ожидается расширение.
...
**A:** `map(Function<T, R>)` — каждый элемент → один элемент типа R. `flatMap(Function<T, Stream<R>>)` — каждый элемент → stream из 0+ элементов R, все объединяются в один поток. Пример: `List<List<String>>` → `flatMap(List::stream)` → `Stream<String>`. Use cases flatMap: «развернуть» nested структуру, опциональный результат через Optional.stream() (Java 9+), парсинг где из одного входа получается несколько выходов.

### Q11: Когда стоит использовать parallel streams, а когда нет?
**A:** **Стоит**: CPU-intensive операции на больших коллекциях (>10k элементов), stateless/associative операции, нет shared state. Например: численные расчёты, агрегация больших массивов. **НЕ стоит**: IO operations (блокируют `ForkJoinPool.commonPool`, голодание других задач), маленькие коллекции (overhead на разбиение > выигрыш), при наличии shared mutable state (race), LinkedList и другие плохо-разбиваемые структуры. Парадокс — `Stream.parallel()` в production коде чаще вредит, чем помогает, потому что разработчики недооценивают разбиение overhead.

### Q12: Когда не нужно использовать Optional?
**A:** Antipatterns: **поля класса** (увеличивает serialization complexity, redundant — null уже нулл), **параметры метода** (use overloading или nullable parameter), **в коллекциях** (`List<Optional<T>>` — уродливо), `Optional.get()` без `isPresent` (NoSuchElementException). Правильно: только как **return type** метода, чтобы явно сказать «может не быть значения». Также — операции на Optional через `map`/`flatMap`/`filter`/`orElseGet` вместо if-else.
...
**A:** Pyramid (Cohn): много **unit** тестов (быстрые, изолированные, моки), умеренно **integration** (с реальными зависимостями: БД, Kafka через TestContainers), мало **E2E** (через UI / API). Ice cream cone — перевёрнутая: много E2E, мало unit. Проблема ICC: тесты медленные (минуты), хрупкие (любое UI-изменение ломает), сложно дебажить (failure mode неясен), стоят дорого в поддержке. Pyramid даёт быструю обратную связь (unit за секунды) + доверие (E2E проверяет важные сценарии целиком).
...
### Q21: Что такое mutation testing и зачем оно если есть code coverage?
**A:** Mutation testing вводит **намеренные ошибки** (мутации) в код: меняет `>=` на `>`, `+` на `-`, удаляет вызов метода. Запускает тесты — если тесты **не падают** (мутант «выжил»), тесты не покрывают этот случай. Mutation score (% killed mutants) — реальный показатель strength. Code coverage показывает, что строки исполнены, но **не** что результат проверен (можно вызвать метод без assert и coverage будет 100%). Mutation testing раскрывает gaps. Tool: PIT (Pitest). Цена: медленнее тестов (запускают весь набор × количество мутаций).

### Q22: Что такое chaos engineering? Какие виды сбоев инжектируют?
**A:** Намеренное введение сбоев в production-like окружении для проверки устойчивости. Принцип Netflix: «It is much better to fail in front of an audience than to fail in production». Виды: **network partition** (Chaos Mesh, tc), **latency injection** (delay + jitter), **pod kill** (Chaos Monkey), **CPU/memory pressure** (stress-ng), **disk failure**. Цель — найти hidden dependencies, weak retry/timeout настройки, missing fallbacks **до того как** инцидент в prod. Инструменты: Chaos Monkey (классика Netflix), Chaos Mesh (K8s native), Litmus, Gremlin (managed). Начинают на staging, в зрелых командах — game days в production.
