...
поэтому замеры **не ломаются**, если системные часы перевели назад (NTP-коррекция, переход
на летнее время). Это решает классическую проблему «отрицательной длительности» из Java до
`System.nanoTime`.
