    ...
    <div>
      ...
      <div className="overflow-hidden rounded-[10px] border border-border">
        <div className="flex items-center gap-3 border-b border-border bg-bg-soft px-3.5 py-1.5">
          ...
          <span className="grp hidden w-[150px] flex-none text-fg-subtle lg:block">Модуль</span>
          ...
          <span className="grp hidden w-[74px] flex-none text-fg-subtle sm:block">Источник</span>
        </div>
        <div className="scroll max-h-[420px] overflow-y-auto">
          {rows.map((r) => (
            <div key={r.id} className="flex items-center gap-3 border-b border-border bg-bg-card px-3.5 py-[9px] last:border-b-0">
              ...
              <span className={`hidden w-[74px] flex-none font-mono text-[11px] sm:block ${r.source === 'MANUAL' ? 'text-accent' : 'text-fg-subtle'}`}>
                {r.source}
              </span>
            </div>
          ))}
